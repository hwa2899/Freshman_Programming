# -*- coding: utf-8 -*-
"""
Created on Thu May 31 10:47:29 2018
...
class Bullet1():
    ...
    def checkCollision(self,aliens,al):
        for opp in aliens:
            xDiff=self.x-opp.x
            yDiff=self.y-opp.y
...
class Game():
    def __init__(self,w,h):
        ...
        self.ship=Ship("ship.png",400,550,0)
        self.bullets=[]
        self.aliens=[]
        self.al=[]
        for i in range(6):
            self.aliens.append(Alien("aliens.png",100+100*i,0))
        for k in range(6):
            self.al.append(Alien("aliens.png",100+100*k,80))
        self.aliensDir=1
        self.aldir=1
    def aliens_move(self):
        ...
        if self.aldir==-1 and len(self.al)>0:
            num=len(self.al)-1
            if self.al[num].x>870:
                self.aldir*=-1
                for a in self.al:
                    a.moveDown()
            else:
                for a in self.al:
                    a.moveRight()
        elif self.aldir==1 and len(self.al)>0:
            if self.al[0].x<30:
                self.aldir*=-1
                for a in self.al:
                    a.moveDown()
                ...
                for a in self.al:
                    a.moveLeft()
    def run_game(self):
        canvas=pygame.image.load("galaxy.png")
        canvas.convert()
        while True:
            self.clock.tick(30)
            ...
            for c in self.al:
                c.blit_me(self.screen)
            for b in self.bullets:
                b.move(self.screen)
                b.checkCollision(self.aliens,self.al)
                if b.y <=0:
