# -*- coding: utf-8 -*-
"""
Created on Fri May  4 22:40:58 2018
...
# -*- coding: utf-8 -*-
"""
Created on Thu May  3 23:13:32 2018
...
#    m3=1.6
    ...
    no=tkinter.Button(f1,text="No",width=10,command=exit)
    no.grid(row=17,column=2,padx=5,pady=5)
#def con():
def check():
    global wrong,wrongbutton,flag
    #check amount
    if money1+money2+money3>money:
        ...
        times.destroy()
        ok.destroy()
        wrong=tkinter.Label(f1,text="Exceeds amount")
        wrong.grid(row=1,column=0,padx=5,pady=5)
        wrongbutton=tkinter.Button(f1,text="OK",width=10,command=window)
